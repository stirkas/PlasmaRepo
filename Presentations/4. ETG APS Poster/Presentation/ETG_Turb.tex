...
\begin{document}
   ...
   form of our model,
      ...
      \end{aligned}
      \end{equation}
   where $\zeta = \nabla^2\Phi$ and $x,y$ subscripts denote partial derivatives.
   \end{frame}
   ...
   \begin{frame}{Pseudo-Spectral Solver}
      \begin{itemize}
         ...
         \vspace{5mm}
         \item The nonlinear term, due to the polarization drift, is responsible for isotropizing the turbulence.
      \end{itemize}
   \end{frame}
      ...
      \begin{columns}
         \column{0.6\textwidth}
            \begin{figure}
               ...
               \caption{Figure shows a non-linear GENE sim initialized with two modes - a default ZF mode ($k_y=0$) and
