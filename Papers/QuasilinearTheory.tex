...
to be much larger than the neoclassical values, and experiments on JET, for instance, indicate a sharp transition from approximately
neoclassical values of $D$ in the core to very anomalous values in the outer region\cite{WessonB}. Turbulent transport is considered
a plausible candidate for explaining this anomalous transport. 
...
velocity into the velocity-space integral, we get the following result for the quasi-linear particle flux,
   ...
   \begin{equation}
      \Gamma_r = \frac{1}{2\pi}\int_{0}^{2\pi} \int_{-\infty}^{\infty}\delta f \frac{\delta E_\theta}{B}d\theta d^{3}v \;.
   \end{equation}
   %
Note there is a subtle point that only the $\theta$-component of $\delta E$ remains because we have dropped the higher order
term of $\mathcal{O}(\epsilon)$ in the cross product with (3.8). We take the distribution function to be the standard adiabatic form for now, with plans
to update the perturbed distribution function with gyrokinetic effects as well as with trapped or passing particle dynamics.
We start by assuming a Maxwellian distribution and calculating the species density by integrating over velocity-space,
with $k_B T_j \Rightarrow T_j$,
...
the approximation that $\frac{q\delta\Phi}{T_j} \ll 1$, and that of quasi-neutrality, i.e. $n_e = n_i$. (2.7) makes clear the following
definition for the total distribution function,
   \begin{equation}
...
\section{Tokamak Geometry}
   \quad For simplicity, we consider an axisymmetric, large aspect-ratio, circular tokamak. Using the Grad-Shafranov
equation, the following definition for the equilibrium magnetic field can be found,
...
with, simplifying to lowest order in $\epsilon$,
   ...
   \end{equation}
   %
The second and third terms on the left-hand side of (4.6) can be simplified to lowest order in $\epsilon$ using (3.1)-(3.4) and
(4.6)-(4.7) as,
...
side of (4.6) because we are interested in linearizing this equation.
...
\subsection{Non-Adiabatic Distribution Function}
   ...
   \end{equation}
   %
Then, simplifying (4.6) using (4.9)-(4.11) and taking the Fourier transform, gives,
...
derivatives into (4.13) then gives,
...
with the following definition,
   ...
   \end{equation}
   %
Then (4.21) can be simplified further and solved for $\delta\widetilde{H}$ if a few physical assumptions are made for electron drift and ITG modes.
...
that of the perturbed modes,
   ...
   \end{equation}
   %
Now, (4.24) and (4.25) allow the left-hand side of (4.21) to be rewritten for ions as,
   \begin{equation}
   ...
   \end{equation}
   %
Finally we can solve for $\delta\widetilde{H}$ explicitly by replacing the left-hand side of (4.21) with (4.26), giving
