...
\subsection{Gyrokinetic Distribution Function}
   ...
   \begin{aligned}
      ...
      -\frac{q_j}{m_j}[-i\omega J_0(z_j)\delta\widetilde{\Phi}\partial_{\epsilon^*}f_M \\
      -\frac{i}{\omega_{cj}}J_0(z_j)\delta\widetilde{\Phi}(\bm{k}\times\bm{\hat{b}})\cdot\frac{d}{dr} f_M\bm{\hat{e}}_r] &=
      i\frac{q_j}{m_j}J_0(z_j)\delta\widetilde{\Phi}[\omega\partial_{\epsilon^*} + \frac{k_\theta}{\omega_{cj}}\frac{d}{dr}]f_M,
   \end{aligned}
   \end{equation}
...
derivatives into (3.2.2) then gives,
   ...
   \begin{equation}
      \Rightarrow\; (-v_\parallel\partial_l + i(\omega - \bar{\omega}_{dj}))\delta\widetilde{H}_0
      = i\frac{q_j}{T_j}J_0(z_j)\delta\widetilde{\Phi}[\omega - \omega_{*j}^T]f_M,
   \end{equation}
   %
...
the perturbed, non-adiabatic distribution function for ions,
   \begin{equation}
      \delta\widetilde{H}_0 = \frac{q_i}{T_i}J_0(z_i)\delta\widetilde{\Phi}
...
or, converting back to $\bm{x}$ coordinates using (3.1.12) for the velocity-space integration to get $n_j$,
   \begin{equation}
      \delta\widetilde{H}_0(\bm{x}) = \frac{q_i}{T_i}J_0(z_i)\delta\widetilde{\Phi}(\bm{x})
...
\subsection{Quasi-linear Anomalous Flux}
   \quad We can now plug our perturbed distribution functions into the quasi-linear estimate for radial flux, (2.1.4). For a
system of ions and electrons with no impurities, the total perturbed distribution function is given as,
   \begin{equation}
