...
quantity. Within our plasma description, the density and velocity contributing to the flux are written as, 
   ...
   \end{equation}
   %
where $\langle ... \rangle_s$ represents a flux-surface average, and terms with $\delta$ tacked on represent fluctuations.
...
\subsection{Anomalous Transport}
   ...
   %
   \begin{equation}
      \Gamma_r = \langle\delta v_r \delta n\rangle_s,
   \end{equation}
   %
noting that we are considering flow in the radial direction, so averaging over the poloidal and toroidal angles (the flux surface)
when considering the tokamak geometry described in the following section. The lowest order term is second-order in perturbed quantities
because $\langle \langle n \rangle_s \delta v_r \rangle_s = \langle n \rangle_s \langle \delta v_r \rangle_s = 0$, since the average of
an averaged quantity is itself, and the average of truly random fluctuations is zero. The density perturbation can be written in terms
of a perturbed distribution function integrated over all velocity space, and the perturbed velocity as an $\bm{E}\times\bm{B}$ drift
...
velocity into the velocity-space integral, we get the following result for the quasi-linear particle flux, 
   %
   \begin{equation}
      \Gamma_r = \langle\int\delta f \frac{c}{B}(\delta\bm{E}\times\bm{\hat{b}}) d^{3}v\rangle_s
               = \frac{c}{2\pi}\int_{0}^{2\pi} \int\delta f \frac{\delta E_\theta}{B}d\theta d\zeta d^{3}v \;.
   \end{equation}
   %
...
and a Fourier transform can be taken to give a flux for each mode,
   ...
   \begin{equation}
      \Gamma_r = \frac{c}{2\pi}\int_{0}^{2\pi} \int\delta\widetilde{f}
                    \frac{(-\nabla_\theta(\delta\widetilde{\Phi}))}{B}d\theta d\zeta d^{3}v,
   \end{equation}
   %
...
the terms in (2.1.3) gives the final result for the flux,
   %
   \begin{equation}
      \Gamma_r = \frac{ik_\theta c}{2\pi B}\int_{0}^{2\pi}\int(\delta\widetilde{f}\delta\widetilde{\Phi}^*-\delta\widetilde{f}^*\delta\widetilde{\Phi})d\theta d\zeta d^{3}v\;.
   \end{equation}
   %
...
with the following definition for the zeroth-order Bessel function,
   ...
   \end{equation}
   %
You can try $\bm{k}_{\perp}$ equally in $\bm{\hat{e}}_1$ and $\bm{\hat{e}}_2$ and see that the integral (3.1.14) ends up over

\noindent$e^{-iz(\frac{1}{\sqrt{2}}\sin\alpha + \frac{1}{\sqrt{2}}\cos\alpha)}$, which still results in $J_0(z)$.
...
and in order to deal with complex variables we can choose to use the general notation,
   ...
   \begin{equation}
      \mathbb{R}e\{\widetilde{A}\} = \frac{1}{2}(\widetilde{A} + c.c.) = \frac{1}{2}(\widetilde{A} + \widetilde{A}^*)
                                   =   \frac{1}{2}\widetilde{A}_{0,\bm{k}}e^{-i\omega t + i\bm{k}\cdot\bm{x}}
                                     + \frac{1}{2}\widetilde{A}_{0,\bm{k}}e^{i\omega t -i\bm{k}\cdot\bm{x}},
   \end{equation}
   %
Using this notation, though I'll drop the $\bm{k}$ subscript at this point, we can find that the term $\widetilde{f}(-\nabla_{\theta}\widetilde{\Phi})$ reduces to,
   %
   \begin{equation}
   \begin{aligned}
      \widetilde{f}(-\nabla_{\theta}\widetilde{\Phi})
         &= \frac{1}{4}(\widetilde{f}+\widetilde{f}^*)(-\nabla_{\theta}(\widetilde{\Phi} + \widetilde{\Phi}^*))
          = ik_{\theta}\frac{1}{4}(\widetilde{f}+\widetilde{f}^*)(\widetilde{\Phi}^* - \widetilde{\Phi}) \\
         &= ik_{\theta}\frac{1}{4}[(Ae^{i\psi}e^{-i\omega t + i\bm{k}\cdot\bm{x}} + Ae^{-i\psi}e^{i\omega t - i\bm{k}\cdot\bm{x}})
                        (Be^{i\phi}e^{-i\omega t + i\bm{k}\cdot\bm{x}} - Be^{-i\phi}e^{i\omega t - i\bm{k}\cdot\bm{x}})] \\
         &= ik_{\theta}\frac{AB}{4}[(e^{i(\psi-\phi)} - e^{-i(\psi-\phi)}) + (e^{i[2(\omega t - \bm{k}\cdot\bm{x})-(\psi+\phi)]} - e^{-i[2(\omega t - \bm{k}\cdot\bm{x})-(\psi+\phi)]})] \\
         &= ik_{\theta}\frac{AB}{2}[i\sin(\psi-\phi) + i\sin(2(\omega t-\bm{k}\cdot\bm{x})-(\psi-\phi))]\;,
   \end{aligned}
   \end{equation}
   %
where $\widetilde{f}$ and $\widetilde{\Phi}$ have been expanded using (A.1) and (A.2),
   ...
   \end{equation}
   %
If we consider the flux-surface average, over $\theta$ and $\zeta$, from the flux equation (2.1.3), and take into account the form of $\widetilde{\Phi}$ in toroidal coordinates
from Appendix B, we get that the second $\sin$ term in (A.4) will disappear because,
   %
   \begin{equation}
      \langle ...\rangle_x = \frac{1}{2\pi}\int_0^{2\pi}\sin(ax+b)dx = 0,\quad a\in\mathbb{Z}_+,\mathbb{Z}_-
   \end{equation}
   %
where $x$ in the integral above can be replaced with either $\theta$ or $\zeta$ when flux-surface averaging, and $a$ would be $m$ or $n$, the toroidal or poloidal mode number.
This means we need only include two of the terms in the first line of (A.4), namely $[\widetilde{\Phi}^*\widetilde{f} - \widetilde{f^*}\widetilde{\Phi}]$,
proving equation (2.1.4).
...
We take,
   %
   \begin{equation}
      \widetilde{\Phi} = \sum_{m=0}^\infty e^{im\theta}\int_{-\infty}^\infty\widetilde{g}(\eta)e^{-i(m-n_0q(r))\eta}d\eta e^{-i\omega t + in\zeta},
   \end{equation}
   %
then $\widetilde{\Phi}\widetilde{\Phi}^*$ in the quasi-linear flux expression becomes,
   %
   \begin{equation}
      \widetilde{\Phi}\widetilde{\Phi}^* = \sum_{m,m'=0}^{\infty}e^{i(m-m')\theta}\int_{-\infty}^{\infty}\int_{-\infty}^{\infty}
      \widetilde{g}(\eta)\widetilde{g}^*(\eta')e^{i(m'-n_0q(r))\eta'}e^{-i(m-n_0q(r))\eta}d\eta d\eta'\;.
   \end{equation}
   %
We can simplify this with the definition of the $\delta$-function,
   %
   \begin{equation}
   \begin{aligned}
      \delta(x-x') &= \frac{1}{2\pi}\int_{-\infty}^{\infty}e^{ik(x-x')}dk, \\
      \delta(x-x') &= \frac{1}{2\pi}\sum_{k=-\infty}^{\infty}e^{ik(x-x')}\;.
   \end{aligned}
   \end{equation}
   %
