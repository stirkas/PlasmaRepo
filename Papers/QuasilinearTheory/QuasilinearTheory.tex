...
Using this notation, though I'll drop the $\bm{k}$ subscript at this point, we can find that the term $\widetilde{f}(-\nabla_{\theta}\widetilde{\Phi})$ reduces to,
   ...
   \begin{aligned}
      \widetilde{f}(-\nabla_{\theta}\widetilde{\Phi})
         ...
         &= ik_{\theta}[(\frac{A}{2}e^{i\psi}e^{-i\omega t + i\bm{k}\cdot\bm{x}} + \frac{A}{2}e^{-i\psi}e^{i\omega t - i\bm{k}\cdot\bm{x}})
                        (\frac{B}{2}e^{i\phi}e^{-i\omega t + i\bm{k}\cdot\bm{x}} - \frac{B}{2}e^{-i\phi}e^{i\omega t - i\bm{k}\cdot\bm{x}})] \\
...
from Appendix B, we get that the second $\sin$ term in (A.4) will disappear because,
   ...
   \end{equation}
   %
where $x$ in the integral above can be replaced with either $\theta$ or $\zeta$ when flux-surface averaging. This means we need only include two of the terms in the
first line of (A.4), namely $[\widetilde{\Phi}^*\widetilde{f} - \widetilde{f^*}\widetilde{\Phi}]$, proving equation (2.1.4).
